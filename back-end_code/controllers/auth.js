...
exports.signup  = async (req, res) => {
    ...
    const ntnExists = await User.findOne({ NTN: req.body.NTN });
    // const ntndb = await ntn.findOne(ntn });
    if (userExists)
    ...
    const ntnfind = await Ntn.findOne({ NTN: req.body.NTN });
        
    if (!ntnfind)
    return res.status(403).json({
        error: 'NTN not registered with fbr!'
    });
...
exports.addntn  = async (req, res) => {
    const user = await new Ntn(req.body);
    await user.save();
    res.status(200).json({ message: 'ntn added.' });
};
